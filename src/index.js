...
if (HREF[1]) {
    ...
    getAndInsertContent(HREF[1], page, refs.gallerySection).then(
        () => (gallery = new SimpleLightbox('.image-card a')),
    );
    current = HREF[1];
...
refs.searchButton.addEventListener('click', e => {
    ...
    history.pushState(null, null, HREF[0] + '?searchQuery=' + current);
    getAndInsertContent(current, page, refs.gallerySection).then(
        () => (gallery = new SimpleLightbox('.image-card a')),
    );
});
        ...
        if (currentPosition - oldPosition > 0) {
            ...
            if (pageHeight * page - currentPosition < 600 && pageHeight > 2000) {
                if (page < 13) {
                    ...
                    debounce(Notiflix.Notify.warning('Вы достигли конца результатов поиска.'), 3000);
                }
            }
        }
        ...
        if (refs.gallerySection.innerHTML) {
            setTimeout(() => {
                if (currentPosition >= document.documentElement.scrollHeight - 1) {
                    Notiflix.Notify.warning('Вы достигли конца результатов поиска.');
                }
            }, 500);
        }
    }),
